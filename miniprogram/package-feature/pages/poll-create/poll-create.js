...
function getSourceInfo(dish, cityName) {
  ...
  if (dish.sourceBucket === "cityExact" && dish.city === city.name) return { sourceKey: "local", sourceLabel: "本地美食" };
  if (
    dish.province === city.province &&
    (dish.sourceBucket === "provinceShared" || dish.sourceBucket === "regionalShared")
  ) return { sourceKey: "province", sourceLabel: "省内美食" };
  if (dish.sourceBucket === "nationalGeneral") return { sourceKey: "common", sourceLabel: "常见美食" };
  return { sourceKey: "other", sourceLabel: "外地美食" };
}
...
Page({
  ...
  onCreatePoll() {
    ...
    wx.navigateTo({ url: `/package-feature/pages/poll-detail/poll-detail?id=${proposalId}` });
  }
});
